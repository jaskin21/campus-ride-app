  ...
  try {
    if (action === 'start') {
      ...
      if (stops.length === 0) {
        return {
          ...
          body: JSON.stringify({ message: 'No active stops found — seed stops first' }),
        }
      }
      ...
      return {
        statusCode: 200,
        headers: { 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ message: 'Simulation started', startStop: firstStop.name }),
      }
    }
    ...
    if (action === 'stop') {
      ...
      return {
        statusCode: 200,
        headers: { 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ message: 'Simulation stopped' }),
      }
    }
